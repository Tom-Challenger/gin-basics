...
func (h *Handler) UpdateEmployee(c *gin.Context) {
	// Получаем значение id из параметра GET запроса
	...
	// Проверяем, что структура Employee 
	// может быть распознана как JSON объект
	var employee Employee
	...
	// Выполняем обращение к хранилищу
	h.storage.Update(id, employee)

	// Возвращаем ответ со статусом 200 и телом в формате JSON
	c.JSON(http.StatusOK, map[string]interface{}{
		"id": employee.ID,
	})
}
...
func (h *Handler) DeleteEmployee(c *gin.Context) {
	...
	// Возвращаем ответ со статусом 200 и телом в формате строки
